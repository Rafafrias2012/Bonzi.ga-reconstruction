"use strict";
function openDMWindow(recipient) {
  var dmWindow = $('<div class="xp_dialog dm_window">' +
    '<div class="xp_titlebar">' +
      '<div class="xp_titlebar_text">Direct Message - ' + recipient + '</div>' +
      '<div class="xp_titlebar_close">X</div>' +
    '</div>' +
    '<div class="dm_messages"></div>' +
    '<input type="text" class="dm_input">' +
    '<button class="dm_send">Send</button>' +
  '</div>');
  
  $('body').append(dmWindow);
  ...
  dmWindow.find('.xp_titlebar_close').click(function() {
    dmWindow.remove();
  });
  
  dmWindow.find('.dm_send').click(function() {
    var message = dmWindow.find('.dm_input').val();
    if (message) {
      sendDM(recipient, message);
      dmWindow.find('.dm_input').val('');
    }
  });
}

function sendDM(recipient, message) {
  socket.emit('dm', { recipient: recipient, message: message });
}
...
function setup() {
    $("#chat_send").click(sendInput),
        ...
        socket.on("background", function(a) {
            document.body.style.backgroundImage = `url(${a.url})`;
        });
  ...
  var dmWindow = $('.dm_window').filter(function() {
    return $(this).find('.xp_titlebar_text').text() === 'Direct Message - ' + data.sender;
  });
  
  if (dmWindow.length === 0) {
    openDMWindow(data.sender);
    dmWindow = $('.dm_window').last();
  }
  
  dmWindow.find('.dm_messages').append('<div>' + data.sender + ': ' + data.message + '</div>');
});
}
...
var _createClass = (function () {
  ...
  name: "Kick",
  visible: function() {
    return d.userPublic.color === "pope" || d.private.runlevel >= 2;
  },
  callback: function() {
        ...
        return (
            _createClass(a, [
                ...
                },
                {
                    ...
                    value: function () {
                        let rolePrefix = "";
  if (this.private.runlevel === 1) rolePrefix = "(Mod) ";
  else if (this.private.runlevel === 2) rolePrefix = "(Admin) ";
  else if (this.private.runlevel === 3) rolePrefix = "(Owner) ";
  this.$nametag.text(rolePrefix + this.userPublic.name);
                    },
                },
                {
                    key: "youtube",
